...
export default function Work() {
  ...
  const services = [
    ...
    },
    {
      ...
      title: "Content Development",
       description: "TWe don't just write words; we craft compelling narratives, build educational resources.",
      image: cont,
    }
  ...
  const nextService = () => {
    setCurrentIndex((prev) =>
      (prev + slidesToShow) % services.length
    );
  };
  ...
  if (visibleSlides.length < slidesToShow) {
    visibleSlides.push(...services.slice(0, slidesToShow - visibleSlides.length));
  }
